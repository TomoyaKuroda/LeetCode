...
var myAtoi = function (s) {
  let findMinusOrPlus = false;
  let output = "";
  for (let i = 0; i < s.length; i++) {
    if (s[i].match(/[a-z]|[A-Z]/)) {
      break;
    } else if ((s[i] === "+" || s[i] === "-") && findMinusOrPlus) {
      break;
    } else {
      if (s[i] === "+" || s[i] === "-") {
        findMinusOrPlus = true;
        if (i > 0 && s[i - 1].match(/\d/)) {
          break;
        }
      }
      if (output !== "" && s[i].match(/[a-z]|[A-Z]|\d/) && s[i - 1] === " " && i > 0) {
        break;
      }
      if (findMinusOrPlus && s[i] == " ") {
        break;
      }
      if (s[i] !== "+" && s[i] !== " ") {
        output += s[i];
